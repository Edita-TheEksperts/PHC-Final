...
def apply_doc_styles(doc):
    """Apply the shared PHC styling — font + page margins."""
...
def add_title_and_vorwort(doc, subtitle="Update- & Test-Dokumentation"):
    """Add the cover page + Vorwort. Caller can pass a different subtitle for
    the FULL variant. Includes its own page break at the end."""
...
def build(doc, include_title=True):
    """Populate `doc` with the audit content. Pass include_title=False to skip
    the cover page + Vorwort (so a caller can supply its own)."""
    apply_doc_styles(doc)
    if include_title:
        add_title_and_vorwort(doc)
